...
// === stałe NA ZEWNĄTRZ (żeby nie powodować re-renderów i uciekania focusa)
const classOptions = [
  'I Liceum',
  'II Liceum',
  'III Liceum',
...
const ControlShell = React.memo(function ControlShell({ icon: Icon, children, hasError }) {
  return (
    <div
      className={[
...
function ContactForm({ hideClass = false, hideCity = false }) {
  ...
  const validateForm = () => {
    ...
    if (!hideCity && !formData.city?.trim()) newErrors.city = 'Miasto jest wymagane';
        ...
        {!hideClass && (
          <Field label="Klasa" required error={errors.class}>
            <ControlShell icon={GraduationCap} hasError={!!errors.class}>
              <select name="class" value={formData.class} onChange={handleChange} className={selectBase}>
